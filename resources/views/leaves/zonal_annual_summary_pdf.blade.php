...
<head>
    ...
    <title>Zonal Annual Staff Leave Summary - {{ $year }}</title>
    <style>
        @page {
            margin: 1cm;
            size: A4 landscape;
...
<body>
    <div class="header">
        <h1>Zonal Annual Staff Leave Summary - {{ $year }}</h1>
        <p>ZONAL EDUCATION OFFICE - DEHIOVITA</p>
        <div style="font-size: 11px; margin-top: 5px;">Generated on: {{ date('Y-m-d H:i') }}</div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">{{ $data['casual'] }}</td>
                <td class="text-center">{{ $data['sick'] }}</td>
                <td class="text-center fw-bold">{{ $data['annual'] }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
