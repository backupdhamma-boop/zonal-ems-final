    ...
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @if(Auth::user()->email === 'admin@admin.com')
                {{ __('Leave Administration - නිවාඩු කළමනාකරණය') }}
            @else
                {{ __('My Dashboard') }}
            @endif
        </h2>
        ...
        @if(Auth::user()->email !== 'admin@admin.com')
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div class="relative">
                            @if(Auth::user()->profile_photo_path)
                                <img src="{{ asset('storage/' . Auth::user()->profile_photo_path) }}" class="rounded-full w-32 h-32 object-cover border-4 border-indigo-100 shadow-md">
                            @else
                        ...
                        <div class="text-center md:text-left min-w-[250px]">
                            <h1 class="text-2xl font-bold text-gray-800">{{ Auth::user()->full_name ?? Auth::user()->name }}</h1>
                            <p class="text-indigo-600 font-semibold text-lg">{{ Auth::user()->designation ?? 'SLEAS / (Assignment Pending)' }}</p>
                            <div class="mt-2 flex flex-wrap justify-center md:justify-start gap-4 text-sm text-gray-600">
                                <span><i class="fas fa-id-badge mr-1 text-gray-400"></i> ID: EMP-{{ str_pad(Auth::user()->id ?? 0, 4, '0', STR_PAD_LEFT) }}</span>
                                <span><i class="fas fa-university mr-1 text-gray-400"></i> {{ Auth::user()->workplace ?? 'MOE / SL' }}</span>
                            </div>
                        </div>
                            ...
                            @foreach([
                                ...
                                <div class="bg-gray-50 rounded-xl p-3 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200">
                                    <div class="flex justify-between items-start mb-2">
                                        <div class="p-2 rounded-lg {{ str_replace('bg-', 'bg-opacity-10 text-', $color) }} {{ $color }}">
                                            <i class="fas {{ $icon }}"></i>
                                        </div>
                                        <span class="text-xs font-bold text-gray-400 uppercase tracking-tighter">{{ $type === 'Total' ? 'ANNUAL OVERALL' : $type }}</span>
                                    </div>
                                    <div class="flex items-end justify-between">
                                        <div>
                                            <span class="text-xl font-black text-gray-800">{{ $used }}</span>
                                            <span class="text-xs text-gray-400 font-bold">/ {{ $limit }}</span>
                                        </div>
                                        <span class="text-[10px] font-bold {{ $percent > 80 ? 'text-red-500' : 'text-gray-400' }}">{{ round($percent) }}%</span>
                                    </div>
                            ...
                            <div class="flex items-center p-3 bg-yellow-50 rounded-lg border border-yellow-100 transition-transform hover:scale-[1.02] duration-200">
                                <div class="w-10 h-10 rounded-full bg-yellow-400 flex items-center justify-center text-white font-bold text-lg shadow-sm mr-3 overflow-hidden">
                                     @if($ub->profile_photo_path)
                                        <img src="{{ asset('storage/' . $ub->profile_photo_path) }}" class="w-full h-full object-cover shadow-sm">
                                    @else
                                        {{ substr($ub->name, 0, 1) }}
                                    @endif
                                </div>
                                <div class="overflow-hidden">
                                    <div class="font-bold text-gray-800 text-sm truncate">{{ $ub->full_name ?? $ub->name }}</div>
                                    <div class="text-xs text-yellow-700 font-bold flex items-center">
                                        <i class="far fa-calendar-alt mr-1"></i> {{ $ub->birthday->format('M d') }}
                                    </div>
                                    <div class="text-[10px] text-gray-400 truncate">{{ $ub->designation ?? 'Colleague' }}</div>
                                </div>
                            </div>
                        @empty
            ...
            <div x-show="tab === 'profile'" x-transition x-cloak class="bg-white shadow-sm sm:rounded-lg p-8 animate-fade-in mb-6">
                <div class="flex justify-between items-center mb-6 border-b pb-4">
                    ...
                    <div class="flex gap-2">
                        <a href="{{ route('my.pdf') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-bold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-800 transition-all duration-150 shadow-sm">
                            <i class="fas fa-file-pdf mr-2"></i> Download My Service Record (PDF)
                        </a>
                        <a href="{{ route('profile.edit') }}" class="inline-flex items-center px-4 py-2 bg-indigo-100 border border-transparent rounded-md font-bold text-xs text-indigo-700 uppercase tracking-widest hover:bg-indigo-200 active:bg-indigo-300 transition-all duration-150 shadow-sm">
            ...
            <div x-show="tab === 'leave_details'" x-transition x-cloak class="animate-fade-in">
                ...
                <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                    <div class="p-6 bg-white border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
                        ...
                        <div class="flex gap-2">
                             <a href="{{ route('leaves.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-bold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 transition duration-150 ease-in-out shadow-sm">
                                <i class="fas fa-plus mr-2"></i> Request Leave
                            </a>
                            <a href="{{ route('leaves.my-summary-pdf') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-bold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 transition duration-150 ease-in-out shadow-sm">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-center bg-gray-50 text-indigo-600">{{ $leave->requested_days }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                        <div class="flex flex-col">
                                            <span class="font-medium">{{ $leave->start_date }}</span>
                                            <span class="text-xs text-gray-400">to {{ $leave->end_date }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                        @if($leave->status === 'Pending')
                                            <a href="{{ route('leaves.edit', $leave->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3" title="Edit"><i class="fas fa-edit"></i></a>
                                        @endif
                                        @if(\Carbon\Carbon::parse($leave->start_date)->isFuture())
                                            <form action="{{ route('leaves.cancel', $leave->id) }}" method="POST" class="inline" onsubmit="return confirm('මෙම නිවාඩු අයදුම්පත අවලංගු කිරීමට ඔබට විශ්වාසද?');">
                                                @csrf
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            ...
                                            @endphp
                                            @if($photo)
                                                <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/' . $photo) }}" alt="">
                                            @else
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $leave->user->full_name ?? $leave->user->name }}</div>
                                            <div class="text-xs text-gray-500">{{ $leave->user->designation ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $leave->leave_type }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-center text-indigo-600">{{ $leave->requested_days }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    <div class="text-xs font-medium">{{ $leave->start_date }}</div>
                                    <div class="text-xs text-gray-400">to {{ $leave->end_date }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($leave->status === 'Pending')
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">Pending</span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    <form action="{{ route('leaves.destroy', $leave->id) }}" method="POST" class="inline" onsubmit="return confirm('මෙම වාර්තාව මකා දැමීමට ඔබට විශ්වාසද?');">
                                        @method('DELETE')
                                        @csrf
