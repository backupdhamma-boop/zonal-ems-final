...
<head>
    ...
    <title>Leave Application - {{ $user->full_name ?? $user->name }}</title>
    <style>
        @page {
            margin: 2cm;
            size: A4;
...
<body>
    ...
    <table>
        ...
        <tr>
            ...
            <td>From <strong>{{ $leave->start_date }}</strong> To <strong>{{ $leave->end_date }}</strong></td>
        </tr>
        <tr>
            <td class="label">Net Business Days (වැඩ කරන දින ගණන)</td>
            <td><strong>{{ $leave->requested_days }}</strong> days</td>
        </tr>
        <tr>
    ...
    <div class="section-title">3. Current Leave Balance (පවතින නිවාඩු ශේෂය - {{ date('Y') }})</div>
    <table class="balance-table">
        ...
        <tbody>
            <tr>
                ...
                <td>{{ $balances['Sick Leave']['used'] }}</td>
                <td>21</td>
                <td>{{ 21 - $balances['Sick Leave']['used'] }}</td>
            </tr>
            <tr>
                <td>Casual Leave (අනියම්)</td>
                <td>{{ $balances['Casual Leave']['used'] }}</td>
                <td>24</td>
                <td>{{ 24 - $balances['Casual Leave']['used'] }}</td>
            </tr>
            <tr>
                <td>Annual Leave (වාර්ෂික)</td>
                <td>{{ $balances['Annual Leave']['used'] }}</td>
                <td>45</td>
                <td>{{ 45 - $balances['Annual Leave']['used'] }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="footer">
        Generated by HR Management System on {{ date('Y-m-d H:i:s') }} | Official Document
    </div>
</body>
</html>
